...
function Services() {
  ...
  const allServices = cmsItems
    ? cmsItems.map((item, i) => {
        // The "Edit existing →" import writes the legacy `cover` field
        // (sometimes as a Cloudinary-shaped object, sometimes as a raw
        // string URL); newer admin uploads write `cover_image`. Accept
        // either, and ALWAYS return an object so consumers never read
        // `item.cover.type` against null.
        const adminCover = item.cover_image || item.cover || null;
        let coverObj = { type: "image", src: "", alt: item.name || item.title || "" };
        if (adminCover && typeof adminCover === "object") {
          const url = adminCover.cloudinary_url || adminCover.src || adminCover.url || "";
          coverObj = {
            type: adminCover.asset_type === "video" || adminCover.type === "video" ? "video" : "image",
            src: url,
            poster: adminCover.poster_url || adminCover.poster || "",
            alt: adminCover.alt || item.name || item.title || "",
          };
        } else if (typeof adminCover === "string") {
          coverObj = { type: "image", src: adminCover, alt: item.name || item.title || "" };
        }
        return {
